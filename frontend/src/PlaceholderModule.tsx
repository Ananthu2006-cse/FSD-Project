  ...
  return (
    <div className="selection:bg-amber-300 selection:text-black">
      <div className="bg-[#fffdf7] border-4 border-stone-900 rounded-lg p-6 shadow-[6px_6px_0px_0px_#1c1917]">
        ...
        {/* Header */}
        <div className="flex items-start justify-between flex-wrap gap-3 pb-4 border-b-2 border-dashed border-stone-300 mb-6">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-stone-900 uppercase mt-1">{title}</h1>
            <p className="font-mono text-xs text-stone-600 uppercase mt-1">{description}</p>
          </div>
        ...
        {/* Users module: real interface */}
        {moduleKey === 'users' ? (
          <div>
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <div>
                <h2 className="text-lg font-black text-stone-900 uppercase">Operator Directory</h2>
                <p className="font-mono text-xs text-stone-600 uppercase">Admin user lifecycle management</p>
              </div>
              <button
                onClick={loadUsers}
            ...
            {loadingUsers ? (
              <div className="py-12 text-center font-mono text-sm text-stone-600">
                <div className="w-8 h-8 border-2 border-stone-900 border-t-amber-500 rounded-full animate-spin mx-auto mb-3" />
                      ...
                      <tr key={u.id} className="hover:bg-amber-50">
                        ...
                        <td className="py-3 px-4">
                          <span className={`px-2 py-0.5 rounded border text-[10px] font-black uppercase ${
                            u.role === 'ADMIN' ? 'bg-red-200 border-red-800 text-red-950'
                            : u.role === 'MANAGER' ? 'bg-blue-200 border-blue-800 text-blue-950'
          ...
          <div>
            <div className="bg-amber-100 border-2 border-stone-900 rounded p-4 mb-6 shadow-[3px_3px_0px_0px_#1c1917]">
              <div className="flex items-center gap-2">
                <span className="font-mono font-bold text-amber-800 text-base">🚧</span>
                <span className="font-mono text-xs font-bold text-stone-900 uppercase">
                  Coming Soon — Module Under Construction
                </span>
              </div>
              <p className="mt-1 text-xs font-mono text-stone-700">
                The <strong className="uppercase">{title}</strong> module is planned for a future phase.
                The route is protected and role-verified. Full functionality will be implemented shortly.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {plannedFeatures.map((feature, idx) => (
                <div key={idx} className="p-4 bg-[#fdfbf7] border-2 border-stone-900 rounded shadow-[3px_3px_0px_0px_#1c1917]">
                  <span className="font-mono text-[10px] font-bold text-stone-500 uppercase tracking-widest block mb-1">
                    Feature 0{idx + 1}
                  </span>
                  <h3 className="font-black text-sm text-stone-900 uppercase mb-2">{feature}</h3>
                  <div className="font-mono text-[11px] text-stone-600 bg-stone-100 p-2 rounded border border-stone-300">
                    Role: <strong className="text-stone-800">{user?.role}</strong> (Authorized)
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="mt-8 pt-4 border-t-2 border-dashed border-stone-300 flex items-center justify-between text-stone-500 font-mono text-[11px] flex-wrap gap-2">
          <span>// RBAC: Route protected via JWT + RoleRoute guard</span>
